...
class MergeManager(SQLManager):
    def merger(self, account_id: int, upload_path: str, destination: str,
               merge_method: str, file_format: str) -> bool:
        # get shop_cart info
        command = (
        ...
        for index, row in shop_cart_survey_problems.iterrows():
            temp_path = upload_path + '/' + str(row['age_type']) + '/' + str(
                row['survey_type']) + '/' + str(row['wave']) + '.sav'
            if temp_path not in file_names:
                ...
                if row['survey_type'] == 1:
                    ...
                    temp_survey = 'friend'
                suffix.append(
                    ('small' if str(row['age_type']) == 1 else 'big') + '_' +
                    temp_survey + '_M' + str(row['wave']))
                used_columns.append(['baby_id'])

            used_columns[file_names.index(temp_path)].append(
                row['problem_name'])
        ...
        if merge_method == 'union':
            ...
            for i in range(len(dataframes)):
                ...
                dataframes[i] = dataframes[i].rename(
                    columns={dataframes[i].columns[0]: 'baby_id'})
            # how can be ['left','right','outer','inner','cross']
            result = reduce(
                lambda left, right: pandas.merge(
                    left, right, on=['baby_id'], how=merge_method), dataframes)
        ...
        for columns in range(len(used_columns)):
            for item in range(len(used_columns[columns])):
                ...
                if column_name == 'baby_id':
                    full_dict.update({
                        'baby_id':
                        metas[columns].variable_value_labels.get('baby_id')
                    })
                elif column_name not in full_dict:
                    if merge_method == 'union':
                        full_dict.update({
                            column_name:
                            metas[columns].variable_value_labels.get(
                                column_name)
                        })
                    else:
                        full_dict.update({
                            column_name + '_' + suffix[columns]:
                            metas[columns].variable_value_labels.get(
                                column_name)
                        })
                else:
        ...
        if file_format == 'sav':
            ...
            pyreadstat.write_sav(
                ...
                variable_value_labels=full_dict)  #,variable_measure=)
            ...
            for file in range(len(used_columns)):
                for column in range(len(used_columns[file])):
                    original_type = metas[file].original_variable_types.get(
                        used_columns[file][column])
                ...
                for file in range(len(used_columns)):
                    for col in range(len(used_columns[file])):
                        ...
                        if current_column != 'baby_id':
                            current_column = current_column + '_' + suffix[file]
                        problem_value_labels.loc[
                            problem_value_labels['problem_id'] ==
                            current_column, 'topic'] = problem_topics.loc[
                                problem_topics['problem_name'] == before,
                                'topic']
            else:
            ...
            with pandas.ExcelWriter(destination,
                ...
                problem_value_labels.to_excel(writer,
                                              index=False,
                                              sheet_name='Value_Labels')
        else:
            # not possible
            return False
