...
class UploadManager(SQLManager):
    ...
    def add_survey(self, survey_info: SurveyInfo):
        check_dupl_op = (
            'SELECT survey_id,age_type, survey_type, wave FROM survey '
        ...
        command = (
            'INSERT INTO survey ( age_type, survey_type, wave,release) '
    ...
    def add_problem(self, meta):
        old_problems = pandas.read_sql('SELECT problem_name FROM dbo.problem;',
                                       self.conn)
        ...
        insert_problems = pandas.concat([given_problems,
                                         old_problems]).drop_duplicates(
                                             subset='problem_name', keep=False)
